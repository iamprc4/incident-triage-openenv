...
def _baseline_action(task_name: str) -> IncidentAction:
    if task_name == "easy_password_reset":
        return IncidentAction(
            summary="User cannot receive MFA and needs verified recovery support.",
            category="auth",
            priority="medium",
            owner_team="support",
            runbook_steps=["verify identity", "mfa reset", "restore access"],
            customer_message="We will verify your identity and help restore access safely.",
        )
    if task_name == "medium_db_latency":
        return IncidentAction(
            summary="Checkout latency regression after migration needs immediate database triage.",
            category="database",
            priority="high",
            owner_team="db",
            runbook_steps=["rollback migration", "analyze slow query", "add index"],
            customer_message="We are investigating checkout performance and will update shortly.",
        )
    return IncidentAction(
        summary="Potential credential compromise requires immediate containment and forensic review.",
        category="security",
        priority="critical",
        owner_team="security",
        runbook_steps=["revoke tokens", "isolate access", "audit logs", "notify legal"],
        customer_message="Containment is in progress for a security incident and updates will follow.",
    )
